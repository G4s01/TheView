...
- `src/lib/server/discovery.ts`: Core logic per interrogare il demone Docker. Le verifiche degli update dei container avvengono ESCLUSIVAMENTE confrontando il digest SHA256 locale con quello remoto (su GHCR o DockerHub), ignorando il `container_id` o i tag semantici.
