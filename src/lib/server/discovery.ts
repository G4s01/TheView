...
export async function getNpmServices(): Promise<DiscoveredService[]> {
	const npmUrl = env.NPM_URL;
	const email = env.NPM_EMAIL;
	const password = env.NPM_PASSWORD;

	if (!npmUrl || !email || !password) {
		console.warn('NPM credentials not configured in .env');
		return [];
	}
	...
	try {
		...
		// 2. Get Proxy Hosts
		const hostsRes = await fetch(`${npmUrl.replace(/\/$/, '')}/api/nginx/proxy-hosts?expand=owner,access_list,certificate`, {
			...
			res.on('end', () => {
				...
				try {
					...
					for (const container of containers) {
						...
						// Skip the dashboard itself ideally, or let user decide
						services.push({
							id: `docker-${container.Id.substring(0, 12)}`,
							name,
							url: '', // Docker itself doesn't know the external URL
							source: 'docker',
...
export async function discoverAllServices(existingUrls: string[]): Promise<DiscoveredService[]> {
	const [npm, docker] = await Promise.all([getNpmServices(), getDockerServices()]);
