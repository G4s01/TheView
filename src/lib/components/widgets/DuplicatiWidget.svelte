<script lang="ts">
	...
	let state = $derived(query.data?.serverState || {});
	let backups = $derived(query.data?.backups || []);
	let progress = $derived(query.data?.progressState || null);
	
	let isServiceActive = $derived(state.ProgramState === 'Running');
	let isPaused = $derived(state.ProgramState === 'Paused');
	let isError = $derived(state.HasWarning || state.HasError);
    let activeTask = $derived(state.ActiveTask);
