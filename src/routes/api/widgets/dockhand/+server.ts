...
async function getAuthCookie(url: string, username?: string, password?: string) {
    if (cachedCookie && cookieExpiry && Date.now() < cookieExpiry) return cachedCookie;
    ...
    const sessionCookie = setCookies.find(c => c && c.startsWith('dockhand_session='));
    if (sessionCookie) {
...
export const GET: RequestHandler = async () => {
    try {
        ...
        if (!url) return json({ error: 'Dockhand non configurato' }, { status: 400 });
        ...
        const res = await undiciFetch(`${normalizedUrl}/api/containers?env=${envId}`, {
            headers: { 'Cookie': cookie },
            dispatcher: agent
        });
        ...
        const updatesRes = await undiciFetch(`${normalizedUrl}/api/containers/check-updates?env=${envId}`, {
            headers: { 'Cookie': cookie },
            dispatcher: agent
        });
        ...
        if (updatesRes.ok) {
            ...
            const pendingIds = new Set(updatesData.pendingUpdates?.map((u: any) => u.containerId) || []);
...
export const POST: RequestHandler = async ({ request }) => {
    try {
        ...
        if (action === 'checkUpdates') {
            ...
            endpoint = `${normalizedUrl}/api/containers/${id}/${action}?env=${envId}`;
            if (action === 'update') {
                if (!payload || !payload.image || !payload.name) {
                    return json({ error: 'Dati payload mancanti per aggiornamento' }, { status: 400 });
                }
                body = JSON.stringify({
                    image: payload.image,
                    name: payload.name,
