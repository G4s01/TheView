...
export const GET: RequestHandler = async ({ url, locals }) => {
  ...
  const requireAuth =
    settings["beszel_require_auth"] === "true" ||
    settings["beszel_require_auth"] === true;
  ...
  try {
    ...
    return json(
      { error: "Errore decrittazione credenziali Beszel" },
      { status: 500 },
    );
  }
  ...
  try {
    ...
    if (!cachedToken || !tokenExpiry || Date.now() > tokenExpiry) {
      let authRes = await undiciFetch(
        `${normalizedUrl}/api/admins/auth-with-password`,
        {
          method: "POST",
      ...
      if (!authRes.ok) {
        authRes = await undiciFetch(
          `${normalizedUrl}/api/collections/users/auth-with-password`,
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ identity: beszel_username, password }),
      ...
      if (!authRes.ok) {
        return json(
          {
            error:
              "Autenticazione Beszel fallita. Controlla URL, Email o Password.",
          },
    ...
    if (sysId) {
      const statsRes = await undiciFetch(
        `${normalizedUrl}/api/collections/container_stats/records?filter=system='${sysId}'&sort=-created&perPage=50`,
        {
      ...
      if (!statsRes.ok)
        return json(
          { error: "Recupero stats container fallito" },
    ...
    const systemsRes = await undiciFetch(
      `${normalizedUrl}/api/collections/systems/records?perPage=500`,
      {
        headers: { Authorization: `Bearer ${cachedToken}` },
        dispatcher: agent,
      },
    ...
    if (!systemsRes.ok) {
      ...
      return json(
        { error: "Recupero sistemi fallito" },
    ...
    return json(
      { error: "Errore di connessione a Beszel", details: e.message },
      { status: 500 },
    );
  }
};
