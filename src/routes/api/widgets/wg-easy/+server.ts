...
export const GET: RequestHandler = async ({ url, locals }) => {
	...
	if (!wgeasy_url || !encryptedPassword) return json({ error: 'Configurazione mancante' }, { status: 400 });

	let password = '';
	try { password = decryptString(encryptedPassword); } catch (e) { return json({ error: 'Errore decrittazione' }, { status: 500 }); }
	...
	try {
        ...
        if (!cookie) return json({ error: 'Autenticazione fallita' }, { status: 401 });

		const clientsRes = await undiciFetch(`${normalizedUrl}/api/wireguard/client`, {
			method: 'GET',
...
export const POST: RequestHandler = async ({ request }) => {
	...
	try {
        ...
        if (!res.ok) {
            ...
            return json({ error: 'Errore azione', details: errText }, { status: res.status });
        }
