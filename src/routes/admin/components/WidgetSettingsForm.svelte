<script lang="ts">
	...
	let componentId = $derived("widget-" + title.toLowerCase().replace(/[^a-z0-9]+/g, '-'));
</script>
...
<li id="{componentId}-row" class="px-6 py-5 hover:bg-muted/50 transition-colors flex flex-col gap-4 cursor-pointer {componentId}-row" onclick={() => isExpanded = !isExpanded}>
	<div class="flex items-center justify-between w-full">
		<div class="flex items-center gap-4 flex-1 min-w-0 mr-4">
			<div class="shrink-0">
				<ServiceIcon {icon} name={title} size="lg" {iconStyle} class="shadow-sm border border-border bg-card" />
			</div>
			<div class="flex flex-col sm:flex-row sm:items-center sm:gap-3 flex-1 min-w-0">
				<p class="text-sm font-semibold text-foreground uppercase tracking-wider truncate">{title}</p>
			</div>
		</div>
			...
			<div class="p-5 bg-card text-card-foreground rounded-xl shadow-lg border border-border relative flex flex-col gap-4" onclick={(e) => e.stopPropagation()} role="presentation">
				<div class="grid grid-cols-1 md:grid-cols-12 gap-4 items-center">
					{#if !hideUsername}
						<div class="md:col-span-6 h-10">
							<TextInput label={usernameLabel} bind:value={username} />
						</div>
					{/if}
					{#if !hidePassword}
						<div class="{hideUsername ? 'md:col-span-12' : 'md:col-span-6'} h-10">
							<TextInput label={passwordLabel} type={showPassword ? "text" : "password"} bind:value={password} class="pr-10">
								<button type="button" onclick={() => showPassword = !showPassword} class="absolute inset-y-0 right-0 pr-3 flex items-center text-muted-foreground hover:text-foreground transition-colors">
									{#if showPassword}
				...
				<div class="grid grid-cols-1 md:grid-cols-12 gap-4 items-center">
					<div class="md:col-span-12 h-10">
						<UrlInput label={urlLabel} bind:value={url} />
					</div>
				</div>

				<div class="flex justify-between items-center mt-2">
					<ToggleInput label="AUTENTICAZIONE ATTESA" bind:checked={requireAuth} />
					<SaveButton class="w-32 h-10" onclick={onSave} isLoading={isSaving} />
				</div>
			</div>
		</div>
	{/if}
