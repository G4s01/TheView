...
# Replace npmScheme and npmHost with npmUrlCombined
content = content.replace("let npmScheme = $state('http://');\n\tlet npmHost = $state('');", "let npmUrlCombined = $state('');")
...
content = content.replace(
    "npmUrl: npmHost ? npmScheme + npmHost : ''",
    "npmUrl: npmUrlCombined"
)

# Now rewrite the HTML layout block
html_start = '<div class="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-300">'
html_end = '{#if npmError}'

if html_start in content and html_end in content:
    start_idx = content.find(html_start)
    end_idx = content.find(html_end)
    
    new_html = """<div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700 overflow-hidden animate-in fade-in slide-in-from-bottom-2 duration-300 mb-8">
	<!-- HEADER: RISULTATI + DISCOVERY BUTTON -->
	<div class="p-6 border-b border-gray-100 dark:border-gray-700 flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white dark:bg-gray-800">
		...
		<div class="flex items-center gap-3">
            <button 
                ...
                class="inline-flex items-center px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white text-sm font-bold uppercase tracking-wider rounded-xl transition-all shadow-md shadow-blue-500/20 disabled:opacity-50"
            >
                {#if isDiscovering}
	...
	<!-- NPM CONFIGURATION ACCORDION / INTEGRATED SECTION -->
	<div class="bg-gray-50 dark:bg-gray-900/50 border-b border-gray-100 dark:border-gray-700 p-6">
		<div class="flex items-center justify-between mb-4">
			<div class="flex items-center space-x-3">
				<img src="https://cdn.simpleicons.org/nginxproxymanager/4B5563" alt="NPM" class="w-6 h-6 opacity-75" />
				<h4 class="text-sm font-bold uppercase tracking-wider text-gray-900 dark:text-gray-300">Integrazione NPM</h4>
			</div>
			<button onclick={() => isNpmEditing = !isNpmEditing} class="text-xs font-bold uppercase tracking-wider text-blue-600 hover:text-blue-700 dark:text-blue-400">
				{isNpmEditing ? (npmUrlCombined ? 'Annulla' : '') : 'Modifica'}
			</button>
		</div>
			...
			<div class="space-y-4" transition:slide>
                ...
                <div class="flex justify-end">
                    <button
                        ...
                        class="px-5 py-2 h-[42px] inline-flex items-center text-white bg-blue-600 font-bold uppercase tracking-wider rounded-xl hover:bg-blue-700 transition-colors text-sm shadow-md shadow-blue-500/30"
                    >
                        Salva & Cerca
                    </button>
                </div>
    ...
    # One last fix, we removed an outer div, so let's check for excess closing divs at the very end
    # The original had <div class="space-y-8...">...</div> which we replaced with ONE card instead of TWO.
    # Wait, the original had two big cards inside a space-y-8 div. Now I combined them into a SINGLE card!
    # Let me ensure the closing divs are correct. I'll just write it and check for build errors.
